...
int main(void){
  ...
  for(int i=0; i<680; i++){
    ...
    if(cnt==4){
      printf("4重となる三角形が見つかりました。\n");
      ans[l][0].x = a.x; ans[l][0].x = a.y;
      ans[l][1].x = b.x; ans[l][1].x = b.y;
      ans[l][2].x = c.x; ans[l][2].x = c.y;
      l++;
    }
  }
  ...
  for(l=0; l<680; l++){
    int k=0;
    while(k++<3) {
      if(ans[l][k].x == 0) break; 
      printf("(%lf, %lf), ", ans[l][k].x, ans[l][k].y);
    }
    printf("\n");
  }
